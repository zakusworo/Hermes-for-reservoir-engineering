...
**Best practice:** split independent scenarios across subagents via `delegate_task`, then aggregate results.
...
![Hermes reservoir workflow](../assets/claude_reservoir_workflow.png)
...
```text
/delegate_task
Goal: Run a parallel fan-out study.
...
The final answer should distinguish technical comparison from decision-making. Hermes can organize the sensitivity study, but the engineer still owns the method choice and the consequence of carrying a screening result into a model.
